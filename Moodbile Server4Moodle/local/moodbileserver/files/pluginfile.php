...
define('NO_MOODLE_COOKIES', true);//ADDED by Moodbile

require_once('../../../config.php');//CHANGED by Moodbile
require_once('../../../lib/filelib.php');//CHANGED by Moodbile
...
//clear database from expired token
$DB->delete_records_select('mbl_tokens', 'timedue < ?', array(time()));

//verify token exists
$result = $DB->get_record('mbl_tokens', array('token'=>$token));
if ($result === false) {
    send_file_not_found();
}

//delete used token
$DB->delete_records('mbl_tokens', array('token'=>$token));

//Check fid
$fid_valid = $result->fid;
if ((int)$fid_valid !== $fileid) {
	send_file_not_found();
}
...
if ($file->get_contextid() != $contextid || $file->get_component() != $component || $file->get_filearea() != $filearea) {
    send_file_not_found();
}
