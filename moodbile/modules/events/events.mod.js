Moodbile.behaviorsPatterns.events = function(){
        ...
        if($('#wrapper').find('.events-'+id).is('.events-'+id)) {
            ...
            $.getJSON("dummie/ws.dum.php?jsoncallback=events", {op: 6}, events = function(json){
                $.each(json, function(i, json){
                    $('#wrapper .events-'+id).append('<div class="event ' + json.id + '"><a href="#"><span class="icon-'+json.type+'"></span>' + json.title + '<em>'+ json.enddata +'</em></a></div>');
                });        
            });
