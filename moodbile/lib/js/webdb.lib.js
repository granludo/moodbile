...
Moodbile.webdb.deleteValues = function(tableName, opts, callback) {
    if(Moodbile.webdb.isCompatible()) {
        var valuesToDelete = [];
        var optsSQL = "";
        ...
        Moodbile.webdb.db.transaction(function(tx) {
            tx.executeSql("DELETE FROM "+tableName+" WHERE "+optsSQL+"", valuesToDelete, null, Moodbile.webdb.onError);
        });
    }
}
