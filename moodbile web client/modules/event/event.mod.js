Moodbile.modules.event = {};

Moodbile.modules.event.status = {
    'dataLoaded': false
};

Moodbile.modules.event.menu = {
    'itemName': 'Events',
    'mainItem': true,
...
Moodbile.modules.event.notification = {
    'method' : 'processedData',
    'data'   : 'assignment'
};

Moodbile.modules.event.dependency = 'course';

Moodbile.modules.event.initBehavior = null;

Moodbile.modules.event.depBehavior = function(context) {
    var context = context || document, data = Moodbile.processedData.assignment;
    
    if(data) {
        Moodbile.modules.event.auxFunc.loadEvents(data);
    }
};

Moodbile.modules.event.auxFunc = {};
Moodbile.modules.event.auxFunc.loadEvents = function(data){
    ...
    $.each(sortedData, function(i, data){
        if(data){
            ...
            currentItem.attr({ 'data-course-id': courseid , 'data-event-id': data.id , 'data-type': 'event'});
    ...
    Moodbile.modules.event.status.dataLoaded = true;
};